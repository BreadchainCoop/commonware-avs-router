...
- **ethereum**: `ghcr.io/breadchaincoop/ethereum:dev` - Runs the local blockchain (Anvil)
- **eigenlayer**: `ghcr.io/breadchaincoop/eigenlayer:dev` - Deploys contracts and registers operators
- **signer**: `ghcr.io/layr-labs/cerberus:0.0.2` - BLS signature service

### What the Setup Provides

- Local blockchain running on port 8545
- Deployed EigenLayer contracts (delegation, strategy, BLS signature checker)
...
3. **Start Services**:
   ...
   This will:
   - Start the Ethereum node (Anvil)
   - Deploy all EigenLayer and AVS contracts
...
For `CONTRIBUTOR_X_KEYFILE`, use the `.nodes/operator_keys/testaccX.private.bls.key.json` keyfiles generated by the Docker setup.
